...
module CompleteTheoryAux(T : THEORY)(M : SafeMap.S with type key = string and type value = T.P.t) : 
  ...
  module rec Implementation : (THEORY with type A.t = T.A.t and type P.t = M.t) = struct
    ...
    let parse x es = 
      match T.parse x es with 
      | Left a -> Left a
      | Right p -> Right (from_p p)
    ...
    let push_back (p : P.t) (a : A.t) : K.Test.t PSet.t =
      let aux (k : string) (v : T.P.t) (acc : K.Test.t PSet.t) = 
        PSet.fold (fun test acc2 ->
          let set = match test.node with 
            | Theory a -> (T.push_back v a)
            | _ -> K.push_back_test (M.singleton k v) test 
          in
          PSet.union acc2 set
        ) acc (PSet.create T.Test.compare)
      in
      (* map back using the K module constructors to ensure hash consing *)
      let ret = P.fold aux p (PSet.singleton ~cmp:T.Test.compare (K.theory a)) in
