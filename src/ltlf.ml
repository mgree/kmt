...
module LTLF(T : THEORY) : (THEORY with type A.t = T.A.t ltlf and type P.t = T.P.t) = struct 

  module rec Implementation : (THEORY with type A.t = T.A.t ltlf and type P.t = T.P.t) = struct
    ...
    let rec subterms (a : K.A.t)  =
      ...
      | Since(x,y) -> PSet.union (K.subterms x) (K.subterms y) |> PSet.add (K.theory a)
