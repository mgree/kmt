...
module Make(Key: CollectionType)(Value : CollectionType) = struct
    ...
    *)
    ...
    type enumeration = End | More of key * value * t * enumeration
    ...
    let equal m1 m2 =
      let rec equal_aux e1 e2 =
        ...
        | (_, End) -> false
        | (More(v1, d1, r1, e1), More(v2, d2, r2, e2)) ->
            Key.compare v1 v2 = 0 && Value.equal d1 d2 &&
            equal_aux (cons_enum r1 e1) (cons_enum r2 e2)
      in equal_aux (cons_enum m1 End) (cons_enum m2 End)

    let show t =
      let aux k v acc = 
        (Key.show k) ^ ": " ^ (Value.show v) ^ (Common.add_sep ", " acc) in 
      "{" ^ (fold aux t "") ^ "}"

    let hash t = 
      let aux k v acc = 
        acc + Key.hash k + Value.hash v
      in
      fold aux t 0
