...
let left x = function 
  | Left x -> x 
  | Right _ -> failwith "Either [left]"

let right x = function 
  | Right x -> x 
  | Left _ -> failwith "Either [right]"
...
let cross_product x y base f = 
    BatSet.PSet.fold (fun a' acc1 -> 
      BatSet.PSet.fold (fun b' acc2 -> 
        BatSet.PSet.add (f a' b') acc2
      ) y acc1
    ) x base
