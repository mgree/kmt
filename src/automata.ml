...
module type AUTOMATA_IMPL = sig
  ...
  module Transitions : (SafeMap.S with type key = Transition.t and type value = StateSet.t)
  type t
  val initial : t -> State.t
  val acceptance : t -> State.t -> K.Test.t 
  val transition : t -> State.t -> Transitions.t 
  val create : State.t -> (State.t -> Transitions.t) -> (State.t -> K.Test.t) -> t
  val print : t -> unit
end
...
module Lazy(K : KAT_IMPL)(S : CollectionType) : (AUTOMATA_IMPL with module K=K and module State=S) = struct 
  module K = K
  module State = S
  ...
  let print (auto : t) = 
    ...
    let print_trans (l,l',a,po) =
      Printf.printf "transition: %s -> %s (%s,%s)\n" (State.show l) (State.show l') (K.Test.show a) (POption.show po) in 
    let print_accept (l,a) = 
      Printf.printf "accept:     %s [%s]\n" (State.show l) (K.Test.show a) in
...
module Automata(K : KAT_IMPL) : (KAT_AUTOMATA with module K=K) = struct
  ...
  let rec derivative_test (test : K.Test.t) : DerivativeSet.t = 
    ...
    | PPar (a,b) -> DerivativeSet.union (derivative_test a) (derivative_test b)
    | PSeq (a,b) -> 
  ...
  let rec derivative (term : K.Term.t) : DerivativeSet.t =
    ...
    | Seq (q,r) -> DerivativeSet.union (dotr (derivative q) r) (dotl (observation q) (derivative r))
  ...
  let rec build_map (term : K.Term.t) (i : int ref) : TestNatMap.t = 
    ...
    | Par(a,b) | Seq(a,b) -> TestNatMap.disjoint_union (build_map a i) (build_map b i)
  ...
  module Theory = struct
    ...
    let initial_subterms (subs : TSet.t) (map : SubtermsMap.t) : SubtermsSet.t = 
      let rec aux subs present_subs implied map = 
        if TSet.is_empty subs then 
          (if TSet.is_empty implied then SubtermsSet.empty() else SubtermsSet.singleton implied)
        else 
    ...
    let test_automata (test : K.Test.t) (chars : ActionSet.t) : TestAutomata.t = 
      ...
      let subs = BatSet.PSet.fold (fun s acc -> TSet.add s acc) subs (TSet.empty()) in
      ...
      let trans (l : TSetOption.t) : TestAutomata.Transitions.t = 
        ...
        let get_transitions_aux (c : K.P.t) (s : K.Test.t) (l : TSet.t) : TSet.t  = 
          ...
          BatSet.PSet.fold (fun a acc -> 
            if is_covered_by a l then TSet.singleton s else acc
          ) tests (TSet.empty ())
        in
        let get_transitions (c : K.P.t) (l : TSet.t) : TestAutomata.StateSet.t = 
            let base = TSet.empty () in 
            let new_state = TSet.fold (fun s acc -> TSet.union acc (get_transitions_aux c s l)) subs base in
        ...
        | None ->
            ...
            let map = TSet.fold (fun sub acc -> 
              SubtermsMap.add sub (get_implied_subs sub subs) acc
            ) subs (SubtermsMap.empty()) in
            ...
            ) initials (TestAutomata.Transitions.empty())
        ...
        | Some l ->
            ...
            ActionSet.fold (fun c acc -> 
              TestAutomata.Transitions.add (K.one (), Some c) (get_transitions c l) acc
            ) chars base
      in
  ...
  module Reindex(A : AUTOMATA_IMPL with module K = K) = struct 
    ...
    let reindex (auto : A.t) : t = 
      ...
      let accept (l : int) = 
        A.acceptance auto (R.get_val indexer l) 
      in
      let trans (l : int) =
        let ts = A.transition auto (R.get_val indexer l) in
        A.Transitions.fold (fun k states acc -> 
  ...
  module Intersection = struct
    ...
    let get_action_map (x : Transitions.t) : ActionMap.t = 
      Transitions.fold (fun (a,po) vs acc -> 
        ...
        with _ ->
          ActionMap.add po (TSPairSet.singleton (a,vs)) acc
      ) x (ActionMap.empty())

    let intersect (pol : t) (pred : t) (placeholder : int) : PairAutomata.t = 
      let cross is js = 
        StateSet.fold (fun i acc ->
          StateSet.fold (fun j acc -> 
            PairAutomata.StateSet.add (i,j) acc
          ) js acc  
        ) is (PairAutomata.StateSet.empty()) 
      in
      let accept (i,j) =
      ...
      let trans (i,j) =
        ...
        if j = 0 then 
          let ret = Transitions.fold (fun (a,_) is' acc1 -> 
            Transitions.fold (fun (b,_) js' acc2 -> 
          ...
          let ret = ActionMap.fold (fun po pairs acc -> 
  ...
  module Determinize = struct
    ...
    let rec show_tree (t : minterm_tree) : string = 
      ...
      | Node(t,ls,left,right) -> 
          Printf.sprintf "Node(%s,%s,%s,%s)" (K.Test.show t) (StateSet.show ls) (show_tree left) (show_tree right)
      ...
      | Node (b, ls, left, right) -> 
          ...
          if sat_pos && K.satisfiable neg then
            (if left = Leaf then 
                ...
                Node(b, ls, refine left a l, refine right a l)
            )
          else t
    ...
    let determinize (auto : t) : NatSetAutomata.t = 
      ...
      let trans (ls : NatSet.t) =
        ...
        if NatSet.equal ls (NatSet.singleton 0) then 
          ...
          let tree = Transitions.fold (fun (a,_) ls acc -> refine acc a ls) t root in 

          let vs = leaves tree in
          let ret = List.fold_left (fun acc ((a,ls) : K.Test.t * StateSet.t) -> 
            (* Printf.printf "Adding transition: %s for %s\n" (K.Test.show a) (StateSet.show ls); *)
            NatSetAutomata.Transitions.add (a,None) (StateSetSet.singleton (convert ls)) acc
          ) (NatSetAutomata.Transitions.empty()) vs in 
          ret
        else
          (* look for same (test,action) pair and move to set of states *)
          let ret = NatSet.fold (fun l (acc : NatSetAutomata.Transitions.t) -> 
            let t = transition auto l in 
            Transitions.fold (fun c state acc -> 
              try 
                let z = NatSetAutomata.Transitions.find c acc in 
                let z = StateSetSet.choose z in 
                let ret = StateSetSet.singleton (NatSet.union (convert state) z) in
                NatSetAutomata.Transitions.add c ret acc
              with _ ->
                NatSetAutomata.Transitions.add c (StateSetSet.singleton (convert state)) acc
            ) t acc
          ) ls (NatSetAutomata.Transitions.empty()) in 
          ret
      in
  ...
  let of_term (term : K.Term.t) : t =
    ...
    let accept (l:int) = 
      if l = 0 then K.zero () 
      else observation (NatTermMap.find l partials)
    in
    let trans (l:int) =
      if l = 0 then 
        ...
        let transitions ((d,l,k):DerivativeSet.elt) acc = 
          ...
          if d = K.zero() then acc 
            ...
            with _ ->
              Transitions.add (d, Some action) (StateSet.singleton l) acc in
    ...
    let auto = TestNatMap.fold (fun theory_test placeholder acc -> 
      let x = Theory.test_automata theory_test chars in 
      debug (fun() -> Printf.printf "Automata for: %s\n" (K.Test.show theory_test));
    ...
    module Equivalence = struct
      ...
      let equivalent (auto1 : t) (auto2 : t) : bool = 
        ...
        Transitions.iter (fun (a,_) l1 -> 
          Transitions.iter (fun (b,_) l2 -> 
        ...
        let push_pairs t1 t2 flip exit = 
          t1 |> Transitions.iter (fun (_,po1) l1 -> 
            let l1 = StateSet.choose l1 in 
            let found = ref false in
            t2 |> Transitions.iter (fun (_,po2) l2 -> 
              ...
              | Some p1, Some p2 ->
                  if p1 = p2 then
                    ...
                    if a1 <> a2 then
                      (debug (fun () -> Printf.printf "Different acceptance: (%d,%d)\n" l1 l2);
                      exit := true)
                    else
                      found := true;
                      let value = if flip then (State l2, State l1) else (State l1, State l2) in
                      (debug (fun () -> 
                          Printf.printf 
                            "  Adding state (%s,%s)\n" 
            ...
            if not !found then 
              let value = if flip then (Garbage, State l1) else (State l1, Garbage) in
              (debug (fun () -> 
                  Printf.printf 
                    "  Adding state (%s,%s)\n" 
                    (fst value |> EqState.show) 
        ...
        if !bad then false 
          ...
          (debug (fun () ->
            Queue.iter (fun (x,y) -> Printf.printf "Initial pair: (%s,%s)\n" (EqState.show x) (EqState.show y)) todo);
          ...
          while not (Queue.is_empty todo) && not !exit do 
            ...
            debug (fun () -> Printf.printf "Looking at: (%s,%s)\n" (EqState.show x) (EqState.show y));
